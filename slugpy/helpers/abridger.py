...
def abridge(script_filepath: Path | str, based_on_labels: bool = False) -> list[str]:
    ...
    for slp in Script(script_filepath, 1, random_start=False, iter_as_dict=False):
        ...
        if based_on_labels and label == "O":
            continue

        indent_or_label = label if based_on_labels else indent
        if prev_indent_or_label is not None and indent_or_label != prev_indent_or_label:
