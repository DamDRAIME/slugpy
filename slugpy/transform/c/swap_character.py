...
class SwapCharacter(Transform):
    ...
    The default `character_sampler` is based on a file containing character names scraped +3000 movies' credits.

    Args:
        character_sampler (Sampler, optional): A sampler for selecting character names. Defaults to CharacterSampler.
        p (float): Probability of applying the transform. Defaults to 0.5.
    """

    def __init__(self, character_sampler: Sampler = CharacterSampler(), p: float = 0.5):
        c = Condition(["C"], exclude=["U", "E", "P", "N"])
        super().__init__(c, p)
        self.sampler = character_sampler
